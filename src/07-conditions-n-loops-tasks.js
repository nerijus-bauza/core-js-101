...
function findFirstSingleChar(str) {
  return str
    .split('')
    .filter((char, _, arr) => arr.indexOf(char) === arr.lastIndexOf(char))[0];
}
...
function isCreditCardNumber(ccn) {
  const cardNumberArr = ccn.toString().split('').map((num) => +num);
  let sum = 0;
  const parity = cardNumberArr.length % 2;
  for (let i = 1; i <= cardNumberArr.length; i += 1) {
    if (i % 2 === parity) {
      sum += cardNumberArr[i - 1];
    } else if (cardNumberArr[i - 1] > 4) {
      sum += 2 * cardNumberArr[i - 1] - 9;
    } else {
      sum += 2 * cardNumberArr[i - 1];
    }
  }
  return sum % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  return temp > 9 ? getDigitalRoot(temp) : temp;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (Object.prototype.hasOwnProperty.call(dict, str[i])) {
      stack.push(str[i]);
    } else {
...
function getCommonDirectoryPath(pathes) {
  let commonDir = '';
  const shortest = Math.min(...pathes.map((el) => el.length));
  for (let i = 0; i < shortest; i += 1) {
    if (pathes.every((el) => el[i] === pathes[0][i])) {
      commonDir += pathes[0][i];
    } else {
      break;
    }
  }
